...
public class ConsultationService : IConsultationService
{
    ...
    public async Task<IEnumerable<ConsultationResponse>> GetNutritionistConsultationsAsync(Guid nutritionistId)
    {
        var consultations = await _context.Consultations
            .Include(c => c.Booking)
                .ThenInclude(b => b.Slot)
        ...
        UpdateConsultationRequest request)
    {
        var consultation = await _context.Consultations
            ...
            .FirstOrDefaultAsync(c =>
                c.Id == consultationId &&
                c.NutritionistId == nutritionistId);

        if (consultation is null)
            throw new InvalidOperationException("Консультация не найдена");

        // Матрица допустимых переходов статусов (из курсовой ФС-STAT-01)
        var allowedTransitions = new Dictionary<ConsultationStatus, List<ConsultationStatus>>
        {
            [ConsultationStatus.Scheduled] = [ConsultationStatus.Completed, ConsultationStatus.NoShow, ConsultationStatus.Cancelled],
    ...
    public async Task<ConsultationResponse?> GetByIdAsync(Guid consultationId, Guid userId)
    {
        ...
        var isNutritionist = nutritionist is not null && consultation.NutritionistId == nutritionist.Id;
    ...
    private static string GetStatusName(ConsultationStatus status) => status switch
    {
    ...
    private static ConsultationResponse MapToResponse(Consultation c) => new()
    {
        Id = c.Id,
